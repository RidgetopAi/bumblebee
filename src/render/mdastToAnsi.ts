 ...
 * ```
 */
export function render(markdown: string, width: number, theme: BumblebeeTheme): string {
 ...
 */
function renderRoot(node: Root, terminalWidth: number, theme: BumblebeeTheme): string {
 ...
 */
function renderNode(node: RootContent, terminalWidth: number, theme: BumblebeeTheme): string {
  switch (node.type) {
    case 'paragraph':
    ...
    case 'list':
      return renderList(node as List, terminalWidth, theme);
    case 'listItem':
      return renderListItem(node as ListItem, terminalWidth, theme);
    case 'blockquote':
      return renderBlockquote(node as Blockquote, terminalWidth, theme);
    case 'code':
 ...
 */
function renderList(node: List, terminalWidth: number, theme: BumblebeeTheme): string {
  const isOrdered = node.ordered;
  const start = node.start || 1;

  const items = node.children.map((item, index) => {
    const bullet = isOrdered ? `${start + index}.` : '•';
    return renderListItemWithBullet(item as ListItem, bullet, terminalWidth, theme);
  });
 ...
 */
function renderListItem(node: ListItem, terminalWidth: number, theme: BumblebeeTheme): string {
 ...
 */
function renderBlockquote(node: Blockquote, terminalWidth: number, theme: BumblebeeTheme): string {
  const content = node.children
 ...
 */
function renderUnknown(node: RootContent, terminalWidth: number, theme: BumblebeeTheme): string {
